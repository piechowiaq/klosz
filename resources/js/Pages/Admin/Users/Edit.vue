<template>
    <layout>
        <div>
            ...
            <trashed-message v-if="user.deleted_at" class="mb-6" @restore="restore(user)"> This user has been deleted.
            </trashed-message>
                ...
                <form @submit.prevent="update">
                    <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
                        ...
                        <text-input v-model="form.last_name" :error="form.errors.last_name"
                                    class="pr-6 pb-8 w-full lg:w-1/2" label="Last Name"/>
                        <text-input v-model="form.email" :error="form.errors.email" class="pr-6 pb-8 w-full lg:w-1/2"
                                    label="Email"/>
                        <text-input v-model="form.phone" :error="form.errors.phone" class="pr-6 pb-8 w-full lg:w-1/2"
                                    label="Phone"/>
                        <select-input v-model="form.role_id" :error="form.errors.role_id"
                                      class="pb-8 pr-6 w-full lg:w-1/2" label="Role">
                            <option :value="null"/>
                            <option v-for="(role, id) in roles" :key="role.id" :value="role.id">{{ role.name }}</option>
                        </select-input>
                        <select-input v-model="form.company_ids" multiple :error="form.errors.company_ids"
                    ...
                    <div class="px-8 py-4 bg-gray-50 border-t border-gray-100 flex items-center">
                        <button v-if="user.id !== 1 || user.id !== 1 && !user.deleted_at" value="Delete"
